...
void TwoPLTest1() {
  ...
  res = lock_mgr.UnlockRow(txn, oid, rid0); // set to shrinking phase
  EXPECT_TRUE(res);
  CheckShrinking(txn);
  CheckTxnRowLockSize(txn, oid, 0, 1);

  try {
    lock_mgr.LockRow(txn, LockManager::LockMode::SHARED, oid, rid0);// shrinking phase can not acquire lock
