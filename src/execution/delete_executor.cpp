...
void DeleteExecutor::Init() {
  ...
  // try to acquire intention exclusive lock on table when init
  try {
...
auto DeleteExecutor::Next([[maybe_unused]] Tuple *tuple, RID *rid) -> bool {
  ...
  while (child_executor_->Next(&to_delete_tuple, &emit_rid)) {// pull tuple need to be deleted, child may be filter executor + seq scan executor
