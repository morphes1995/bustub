...
auto NestedLoopJoinExecutor::Next(Tuple *tuple, RID *rid) -> bool {
  RID emit_rid{};
  while (right_tuple_idx_ >= 0  // continue to deal with current left_tuple_
         || left_executor_->Next(&left_tuple_, &emit_rid)) { // fetch next left_tuple_
    std::vector<Value> vals;
    for (uint32_t ridx = (right_tuple_idx_ < 0 ? 0 : right_tuple_idx_); ridx < right_tuples_.size(); ridx++) { // consider each tuple in the join's inner table
      auto &right_tuple = right_tuples_[ridx];
      if (Matched(&left_tuple_, &right_tuple)) { // if match, emit a result tuple
        ...
        *tuple = Tuple(vals, &GetOutputSchema()); // generate a match
        right_tuple_idx_ = ridx + 1;
        return true;
      }
    }
    // there is no match tuple in the inner table.  for left out join, only return tuple in left table, and return null as inner table part
    ...
    // reset right_tuple_index, next loop deal with next left table tuple
    right_tuple_idx_ = -1;
  }
  return false;
}
