...
auto NestIndexJoinExecutor::Next(Tuple *tuple, RID *rid) -> bool {
  ...
  while (child_->Next(&left_tuple, &emit_rid)) {
    // KeyPredicate may be a ColumnValueExpression
    Value value = plan_->KeyPredicate()->Evaluate(&left_tuple, child_->GetOutputSchema());
    std::vector<RID> rids;
    // using outer table attr to loop up inner table index, to get matched inner tuple rid
    ...
    if (!rids.empty()) {
      ...
      //  The output schema of NestedIndexJoin is all columns from the left table (child, outer)
      //  and then from the right table (index, inner)
