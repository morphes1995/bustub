...
void HashJoinExecutor::Init() {
  ...
  // build hash table based on right executor_
  Tuple tmp_tuple{};
  RID rid;
  ...
  while (left_executor_->Next(&tmp_tuple, &rid)) {
    ...
    if (hash_join_table_.count(HashUtil::HashValue(&join_key)) > 0) {
      ...
      for (const auto &tuple : right_tuples) {
        ...
        // two key's underlay bytes is same, but when interpreted as different type, they may be different
        // eg. same 64bits sequence interpreted as bigint_ & timestamp_, logically is different
      ...
      // padding null column when right table no match tuples
      for (uint32_t col_idx = 0; col_idx < plan_->GetRightPlan()->OutputSchema().GetColumnCount(); col_idx++) {
        values.push_back(
